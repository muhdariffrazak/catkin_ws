...
waypoint_saved = False  # Flag to track if the waypoint has been saved
...
def odom_callback(msg):
    global waypoint_saved  # Use the global flag

    if not waypoint_saved:  # Only proceed if the waypoint hasn't been saved yet
        ...
        file_path = '//home/xrrobot/catkin_ws/src/xrrobot_project/xrrobot/script/waypoints.csv'
        ...
        waypoint_saved = True  # Update the flag to indicate the waypoint is saved

if __name__ == "__main__":
    # Initialize the ROS node with a unique name (e.g., "waypoint_saver")
    rospy.init_node("waypoint_saver", anonymous=True)

    # Create the ROS subscriber to listen to the "odom" topic
